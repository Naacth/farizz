...
<form action="/admin/education/store" method="POST">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
            ...
            <select name="level" required 
                ...
                <option value="SD" <?= old('level') === 'SD' ? 'selected' : '' ?>>SD (Elementary)</option>
                <option value="SMP" <?= old('level') === 'SMP' ? 'selected' : '' ?>>SMP (Junior High)</option>
                <option value="SMA" <?= old('level') === 'SMA' ? 'selected' : '' ?>>SMA (Senior High)</option>
                <option value="Kuliah" <?= old('level') === 'Kuliah' ? 'selected' : '' ?>>Kuliah (University)</option>
            </select>
        </div>
    ...
    <div class="mt-6">
        ...
        <textarea name="description" rows="4" 
                  ...
                  class="w-full px-4 py-2 border border-sky-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-transparent"><?= old('description') ?></textarea>
    </div>
